...
function Quiz() {
    ...
    const nextQuestion = () => {
        if (selectedOption === currentQuestion.correctAnswer.toString()) { // change here
                ...
                <>
                    ...
                    <Grid container spacing={2} justifyContent="center">
                        {currentQuestion.options.map(option => (
                            <Grid item xs={12} sm={6} md={3} key={option.id} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                <FormControlLabel
                                    value={option.id.toString()}
                                    control={<Radio />}
                                    label={<img src={option.src} alt={option.alt} style={{ width: '100%', height: 'auto', border: '2px solid #ccc' }} />}
